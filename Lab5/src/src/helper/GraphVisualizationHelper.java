...
public class GraphVisualizationHelper {
  public static void visualize(ConcreteGraph g) throws Exception {
    ...
    for (Edge e : g.edges()) {
      if (e instanceof HyperEdge) {
        for (int i = 0; i < e.getList().size() - 1; i++) {
          Edge edge = new HyperEdge(e.getLabel() + ":" + e.getList().get(i).getLabel() + ","
              + e.getList().get(i + 1).getLabel(), -1);
        ...
        graph.addEdge(e, e.getList().get(0), e.getList().get(1));
    ...
    Transformer<Edge, String> edgeLabel = new Transformer<Edge, String>() {
      ...
      public String transform(Edge p) {
        if (p instanceof DirectedEdge && p.getWeight() != -1) {
          try {
            return p.getLabel() + ":" + p.getList().get(0).getLabel() + "->"
                + p.getList().get(1).getLabel() + ":" + String.format("%.2f", p.getWeight());
          } catch (Exception e) {
    ...
    // @SuppressWarnings("unused")
    // Transformer<Edge, Stroke> edgeStrokeTransformer = new Transformer<Edge,
    // Stroke>() {
