...
public class SocialNetwork {
    ...
    public static List<String> influencers(Map<String, Set<String>> followsGraph) {
    	...
    	for(String key:followsGraph.keySet()) {
    		for(String fname:followsGraph.get(key)) {
    			if(name.containsKey(fname)) {
    				name.put(fname, name.get(fname)+1);
    			}
    			else {
    				name.put(fname, 1);
    			}
    	...
    	while(!name.isEmpty()) {
    		for(String key:name.keySet()) {
        		if(name.get(key)>=temp) {
        			temp=name.get(key);
        			nametemp=key;
        		}
        	}
    		list.add(nametemp);
    		name.remove(nametemp);
    		temp=0;
    	}
    	return list;
    	}
